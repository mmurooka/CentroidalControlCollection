...
TEST(TestPolynomialZmpLaplaceTransform, Test1)
{
  ...
  CCC::PolynomialZmpLaplaceTransform pzlt(com_height);
  ...
  // Setup dump file
  std::string file_path = "/tmp/TestPolynomialZmpLaplaceTransform.txt";
  ...
  while(t < end_time)
  {
    ...
    CCC::PolynomialZmpLaplaceTransform::InitialParam initial_param =
        sim.state_.pos() + std::sqrt(com_height / CCC::constants::g) * sim.state_.vel();
    const auto & ref_data = footstep_manager.makePolynomialZmpLaplaceTransformRefData(t, horizon_duration);
    planned_zmp = pzlt.planOnce(ref_data, initial_param, t);
    ...
    ofs << t << " " << sim.state_.pos().transpose() << " " << planned_zmp.transpose() << " "
        << zmp_limits[0].transpose() << " " << zmp_limits[1].transpose() << " " << initial_param.transpose() << " "
